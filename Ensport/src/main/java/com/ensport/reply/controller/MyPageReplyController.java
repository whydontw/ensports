package com.ensport.reply.controller;
...
public class MyPageReplyController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// listCount 현재 게시글 개수
		replyListCount = new ReplyService().replyListCount(userId); // 게시글 개수 조회해오기
		// 현재페이지 정보(번호)
		...
		//myPage Reply List
		ArrayList<Board> selectMyReplyList = new ReplyService().selectMyReplyList(pi, userId);
		
		
		request.setAttribute("pi", pi);
		request.setAttribute("replyList", selectMyReplyList);
