...
/**
 * Servlet implementation class BaseballSeoulController
 */
@WebServlet("/ba.seoul")
public class BaseballSeoulController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public BaseballSeoulController() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		PlaceAllListCount = new BaseballMatchingService().allSeoulListCount();
		...
		ArrayList<BaseballMatching> list = new BaseballMatchingService().selectAllSeoulSoccerMatchingList(localName,pi);

		request.setAttribute("slist", list);
		request.setAttribute("pi", pi);
		request.getRequestDispatcher("views/matching/baseballSeoul.jsp").forward(request, response);
	}
