...
/**
 * Servlet implementation class BaseballMatchingEnrollController
 */
@WebServlet("/matching.enba")
public class BaseballMatchingEnrollController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public BaseballMatchingEnrollController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//session에서 userNo 뽑아주기(PLAYER TABLE에 INSERT될 친구)
				...
				if(player>=total) {
					...
					int duplicate = new BaseballMatchingService().BaseballMatchingDuplicate(userNo,timeNo, placeNo, reservationDate);
					...
					if(duplicate>0) {
						...
						int result = new BaseballMatchingService().BaseballMatchingReservation(userNo, timeNo, placeNo, reservationDate);
