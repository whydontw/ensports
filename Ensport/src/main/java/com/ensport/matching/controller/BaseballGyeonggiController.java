...
/**
 * Servlet implementation class BaseballGyeonggiController
 */
@WebServlet("/ba.gyeonggi")
public class BaseballGyeonggiController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public BaseballGyeonggiController() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		PlaceAllListCount = new BaseballMatchingService().allGyeonggiListCount();
		...
		ArrayList<BaseballMatching> list = new BaseballMatchingService().selectAllGyeonggiSoccerMatchingList(localName,pi);

		for (BaseballMatching sm : list) {
			System.out.println(sm);
		}

		request.setAttribute("slist", list);
		request.setAttribute("pi", pi);
		request.getRequestDispatcher("views/matching/baseballGyeonggi.jsp").forward(request, response);
	}
